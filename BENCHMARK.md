...
|------|---------|-----------|
| バイナリサイズ | 4.3 MB | 14 MB |
| 起動時間（10回平均） | 0.117秒 | 0.115秒 |
| メモリ使用量（5回平均） | 61 MB | 75 MB |
...
- メモリ使用量: UmiTermが19%少ない（61 MB vs 75 MB）
- バイナリサイズ: UmiTermが1/3（4.3 MB vs 14 MB）
- 起動時間: ほぼ同等（差2ms）
- コード行数: UmiTermが1/12（~2,500行 vs ~30,000行）
